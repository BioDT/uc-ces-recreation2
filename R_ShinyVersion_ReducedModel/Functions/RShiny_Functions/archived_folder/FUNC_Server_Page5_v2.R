server_page5 <- function(input, output, session, all_form_data, form_data, csv_path, persona_id_global) {
  ...
  observeEvent(input$submit, {
    if (persona_id_global$input_text == "") {
    ...
    # Handle renaming of the persona_id
    new_persona_id <- input$rename_persona_id
    if (new_persona_id != "") {
      persona_id_global$input_text <- new_persona_id
    }
    
    # Add a new column for the persona_id_global$input_text if it doesn't exist
    if (!(persona_id_global$input_text %in% colnames(form_data_copy))) {
      form_data_copy[[persona_id_global$input_text]] <- NA
    }
    ...
    for (i in 1:nrow(all_form_data$data)) {
      ...
      if (any(matching_row)) {
        form_data_copy[matching_row, persona_id_global$input_text] <- response
      }
    }
    ...
    write.csv(form_data_copy, 
              paste0("/data/notebooks/rstudio-rpmodel/testp/R_ShinyVersion_ReducedModel/output/Persona_Scores/", persona_id_global$input_text, ".csv"), 
              row.names = FALSE, na = "")
  ...
  output$export_responses <- downloadHandler(
    filename = function() {
      paste0(persona_id_global$input_text, "_final_responses.csv")
    },
