...
reclassify_rasters <- function(cropped_rasters, raster_folder, Score_column) {
  ...
  for (raster_file in names(cropped_rasters)) {
    ...
    # Construct the path to the original raster file
    #original_raster_file <- file.path(raster_folder, raster_file)
    
    # Construct the path to the CSV file
    csv_file <- file.path(raster_folder, paste0(file_path_sans_ext(raster_file), ".csv"))
