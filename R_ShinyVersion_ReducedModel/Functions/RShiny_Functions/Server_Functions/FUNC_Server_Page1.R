...
server_page1 <- function(input, output, session, shapefile_name_global, home_folder, upload_dir) {
  ...
  observeEvent(input$upload, label = "when uploading a new shapefile", {

    if (isTruthy(input$shapefile) && isTruthy(input$shapefile_name)){
  ...
  observeEvent(input$load_existing, label = "when loading an existing file",  {
  ...
  output$map <- renderLeaflet({
    ...
    leaflet() %>%
      addTiles() %>%
      fitBounds(lng1 = bbox_list$xmin, lat1 = bbox_list$ymin, 
