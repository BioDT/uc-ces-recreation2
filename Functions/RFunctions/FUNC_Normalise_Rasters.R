...
normalise_rasters <- function(SpatRaster_file, reference_raster) {
    # Add the sum of the values to the reference raster
    reference_raster <- terra::app(SpatRaster_file, sum, na.rm = TRUE)
    # Normalize the reference raster to a scale of 0-1
    reference_raster <- (reference_raster - min(reference_raster[], na.rm = TRUE))/(max(reference_raster[], na.rm = TRUE) -
        min(reference_raster[], na.rm = TRUE))
...
# normalise_rasters <- function(SpatRaster_file, reference_raster) {
#   raster <- terra::resample(SpatRaster_file, reference_raster, method = 'bilinear')
#   # Add the sum of the values to the reference raster
#   reference_raster <-  terra::app(SpatRaster_file, sum, na.rm = TRUE)
#   # Normalize the reference raster to a scale of 0-1
