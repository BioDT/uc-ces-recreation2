process_area_of_interest <- function(
    Raw_Shapefile, Boundary_name, boundary_dir, resolution = 20, max_distance = 1500
) {

    # Find shapefiles in the directory that include the text in Boundary_name, case-insensitive
    shapefiles <- list.files(
        path = Raw_Shapefile, pattern = paste0(".*", Boundary_name, ".*\\.shp$"),
    ...
    shapefile_27700 <- st_transform(
        shapefile, crs = "+proj=tmerc +lat_0=49 +lon_0=-2 +k=0.9996012717 +x_0=400000 +y_0=-100000 +datum=OSGB36 +units=m +no_defs",
        SHPT = "POLYGON"
    )
    ...
    if (file.exists(file.path(boundary_dir, "boundaries.shp"))) {
        # Delete the existing shapefile
        file.remove(file.path(boundary_dir, "boundaries.shp"))
    }
    # Save the converted shapefile, overwriting existing files
    st_write(
        shapefile_27700, file.path(boundary_dir, "boundaries.shp"),
        overwrite = TRUE
    )
    ...
    empty_raster <- raster::raster(ext, res = resolution, crs = crs)

    # Rasterize the shapefile using the numeric field
    rasterized <- raster::rasterize(shapefile_27700, empty_raster, field = "id")

    # Save the rasterized shapefile
    writeRaster(
        rasterized, file.path(boundary_dir, "empty.tif"),
        format = "GTiff", overwrite = TRUE
    )

    # Load the rasterized shapefile as Raster_Empty
    Raster_Empty <- raster::raster(file.path(boundary_dir, "empty.tif"))
    values(Raster_Empty) <- 0

    # Load the boundaries shapefile as mask_boundary
    target_shapefile <- file.path(boundary_dir, "boundaries.shp")
