process_area_of_interest <- function(Raw_Shapefile, Boundary_name, boundary_dir, resolution = 20, max_distance = 1500) {
  ...
  st_write(shapefile_27700, file.path(boundary_dir, "boundaries.shp"), overwrite = TRUE)
  ...
  writeRaster(rasterized, file.path(boundary_dir, "empty.tif"), format = "GTiff", overwrite = TRUE)
