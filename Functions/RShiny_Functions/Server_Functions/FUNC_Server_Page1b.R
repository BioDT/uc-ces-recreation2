# server functions for creating/selecting persona names

server_page1b <- function(
    input, output, session, persona_id_global, skip_to_page6, home_folder, persona_dir
) {
    ...
    observeEvent(
        input$submit_persona_id, {
            ...
            print(paste("Incoming global Persona ID (created): ", persona_id_global$input_text))  # debugging script
        }
    )
    ...
    observeEvent(
        input$load_existing_persona, {
            ...
            lapply(
                names(question_responses),
                function(q) {
                  updateNumericInput(session, q, value = question_responses[[q]])
                }
            )
            skip_to_page6$skip <- TRUE  # Existing persona, skip to page 6
            print(paste("Incoming global Persona ID (existing): ", persona_id_global$input_text))  # debugging script
        }
    )
