server_page5 <- function(input, output, session, all_form_data, form_data, csv_path, home_folder) {
    ...
    observeEvent(
        input$submit, {
            ...
            page5_data <- data.frame(Name = cleaned_names, Response = all_responses_flat, stringsAsFactors = FALSE)
            ...
            write.csv(
                form_data_copy, paste0(home_folder, "Data/input/Persona_Loaded/", input$persona_id, ".csv"),
                row.names = FALSE, na = ""
            )
