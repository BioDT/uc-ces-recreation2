...
server_page6 <- function(input, output, session, shapefile_name_global, persona_id_global, home_folder,
  ...
  # Run the model
  observeEvent(input$run_model, label = "Running the model", {
    
    #showNotification("Running the model...", type = "message")
    waiter::waiter_show(html = tagList(waiter::spin_fading_circles(),"0/11 preparing files to run the model....."))
    #housekeeping
    ...
    waiter::waiter_hide()
    waiter::waiter_show(html = tagList(waiter::spin_fading_circles(),"2/11 defining the area of interest"))
    ...
    #print how big it is
    rv_local$area <- round(as.numeric(sf::st_area(mask_boundary)/1000000),2)
    ...
    end_time_0 <- Sys.time()
    print(paste("Process the area of interest took", calculate_time_diff(start_time_0, end_time_0)))
    ...
    #################################################################################
    waiter::waiter_hide()
    waiter::waiter_show(html = tagList(waiter::spin_fading_circles(),"3/11 processing the persona specific scores"))
    ...
    print(paste("process the persona scores took: ", calculate_time_diff(start_time_recl, end_time_recl)))
    ...
    print(paste("process the proximity took: ", calculate_time_diff(start_time_dist, end_time_dist)))
    ...
    print(paste("normalising the rasters took: ", calculate_time_diff(start_time_norm, end_time_norm)))
    ...
    #################################################################################
    waiter::waiter_hide()
    waiter::waiter_show(html = tagList(waiter::spin_fading_circles(),"6/11 masking to specific shape"))
    ...
    print(paste("masking the rasters took: ", calculate_time_diff(start_time_mask, end_time_mask)))
    
    
    waiter::waiter_hide()
    waiter::waiter_show(html = tagList(waiter::spin_fading_circles(),"7/11 masking to specific shape"))
    ...
    #################################################################################
    waiter::waiter_hide()
    waiter::waiter_show(html = tagList(waiter::spin_fading_circles(),"8/11 removing excess data from memory"))
    ...
    print(paste("removing excess data from memory took: ", calculate_time_diff(start_time_remove, end_time_remove)))
    ...
    if ( rv_local$area >= area_too_big){
      waiter::waiter_hide()
      waiter::waiter_show(html = tagList(waiter::spin_fading_circles(),paste0("9/11 the area of this case is ", round(rv_local$area, 2), " which is larger than ", area_too_big, " so, one extra step is needed")))
      print(paste0("the area of this case is ", round(rv_local$area, 2), " which is larger than ", area_too_big, " so, one extra step is needed"))
      ...
      terra::writeRaster(Norm_rasters, paste0(temp_folder, "Norm_rasters.tif"), overwrite=TRUE)
      #remove objects from memory
      rm(Norm_rasters, SLSRA_Norm, FIPS_N_Norm, FIPS_I_Norm, Water_Norm)
      
      end_time_extra <- Sys.time()
      print(paste("solving memory issue took: ", calculate_time_diff(start_time_extra, end_time_extra)))
    }
    ...
    ###############################################
    waiter::waiter_hide()
    waiter::waiter_show(html = tagList(waiter::spin_fading_circles(),"10/11 normalize all components"))
    ...
    end_time_5 <- Sys.time()
    print(paste("Compute and normalise final RP Model", calculate_time_diff(start_time_5, end_time_5)))
    ...
    ###############################################
    end_time_g <- Sys.time()
    print(paste("finished, total runtime was ", calculate_time_diff(start_time_g, end_time_g)))
    waiter::waiter_hide()
  ...
  output$map_output <- renderLeaflet({
    ...
    quantiles <- quantile(terra::values(raster_data), probs = seq(0, 1, length.out = 8), na.rm = TRUE)
    ...
    leaflet() %>%
      ...
      leaflet.extras::addDrawToolbar(
        ...
        polygonOptions = leaflet.extras::drawPolygonOptions(shapeOptions = leaflet.extras::drawShapeOptions(color = "blue")),
      ...
      addLegend(
        ...
        title = "Quantile Categories",
        labFormat = labelFormat(digits = 2),  # Optional: Format labels with two decimal places
        opacity = 1
      )
  })
     ...
     raster_crs <- terra::crs(rv_local$raster_data)
      ...
      showNotification(
        paste("Clicked at Longitude:", click$lng,
              "Latitude:", click$lat,
              "Raster Value:", raster_value$sum, collapse = ),
        type = "message"
      )
    }
  })
