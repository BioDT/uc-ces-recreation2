...
server_page6 <- function(input, output, session, shapefile_name_global, persona_id_global) {
    ...
    observe({
      ...
      # Ensure the file extension is removed before further processing
      shapefile_name <- tools::file_path_sans_ext(shapefile_name)
      
      print(paste("Inside observe: Shapefile Name:", shapefile_name))  # debug output
      print(paste("Inside observe: Persona ID:", persona_id))  # debug output
      
      if (!is.null(shapefile_name) && shapefile_name != "" && !is.null(persona_id) && persona_id != "") {
    ...
    observeEvent(input$run_model, {
      ...
      # Ensure the file extension is removed before further processing
      shapefile_name <- tools::file_path_sans_ext(shapefile_name)
      shapefile_name <- basename(shapefile_name)
      
      print(paste("Shapefile Name:", shapefile_name))  # debug output
      print(paste("Persona ID:", persona_id))  # debug output
    ...
    Raw_Shapefile <- file.path(input_folder, "Raw_Shapefile", Boundary_name, paste0(Boundary_name, ".shp"))
    resolution <- 20
    max_distance <- 1500
